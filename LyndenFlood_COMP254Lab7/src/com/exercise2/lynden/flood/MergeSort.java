...
class MergeSort {
  ...
  public static <K> Queue<K> bottomUpMergeSort(K[] items, Comparator<K> comp) {
    Queue<Queue<K>> queueOfQueues = new LinkedQueue<>();

    // Put each item in its own queue
    for (K item : items) {
      Queue<K> singleQueue = new LinkedQueue<>();
      singleQueue.enqueue(item);
      queueOfQueues.enqueue(singleQueue);
    }

    // Repeatedly merge pairs until one queue remains
    while (queueOfQueues.size() > 1) {
      Queue<Queue<K>> nextRound = new LinkedQueue<>();

      while (queueOfQueues.size() > 1) {
        Queue<K> q1 = queueOfQueues.dequeue();
        Queue<K> q2 = queueOfQueues.dequeue();
        Queue<K> merged = new LinkedQueue<>();
        merge(q1, q2, merged, comp); // Use existing merge method
        nextRound.enqueue(merged);
      }

      // Handle odd queue
      if (!queueOfQueues.isEmpty()) {
        nextRound.enqueue(queueOfQueues.dequeue());
      }
...
//	  mergeSort(S, comp);
    ...
    Queue<Integer> bottomUpResult = bottomUpMergeSort(S, comp);
    System.out.println();

    while (!bottomUpResult.isEmpty()){

      System.out.print(bottomUpResult.dequeue() + " ");
    }
