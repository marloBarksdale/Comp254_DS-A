...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
  ...
  /**
   * Computes and prints element and height for every position using postorder traversal
   * Time Complexity: O(n) where n is the number of nodes
   * Space Complexity: O(h) where h is the height of the tree (due to recursion stack)
   */
  public void printElementsWithHeight() {
    System.out.println("Element : Height");
    System.out.println("================");
    if (root != null) {
      computeHeightPostorder(root);
    }
  }

  /**
   * Helper method that performs postorder traversal while computing heights
   * Returns the height of the subtree rooted at node
   */
  private int computeHeightPostorder(Node<E> node) {
    if (node == null) {
      return -1; // Height of empty subtree is -1
    }

    // Process left subtree first (postorder)
    int leftHeight = -1;
    if (node.getLeft() != null) {
      leftHeight = computeHeightPostorder(node.getLeft());
    }

    // Process right subtree second (postorder)
    int rightHeight = -1;
    if (node.getRight() != null) {
      rightHeight = computeHeightPostorder(node.getRight());
    }

    // Compute height of current node's subtree
    int currentHeight = 1 + Math.max(leftHeight, rightHeight);

    // Print element and its height (postorder visit)
    System.out.println(node.getElement() + " : " + currentHeight);
  ...
  public static void main(String[] args) {
    ...
    Position<String> nodeC = tree2.addRight(nodeA, "C");
    Position<String> nodeD = tree2.addLeft(nodeB, "D");
    Position<String> nodeE = tree2.addRight(nodeB, "E");
    Position<String> nodeF = tree2.addRight(nodeC, "F");
    Position<String> nodeG = tree2.addLeft(nodeE, "G");
    ...
    System.out.println("Computing heights using postorder traversal:");
    tree2.printElementsWithHeight();

    System.out.println("\nTime Complexity: O(n) - visits each node exactly once");
    System.out.println("Space Complexity: O(h) - recursion stack depth equals tree height");
  }
